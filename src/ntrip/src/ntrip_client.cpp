...
namespace ntrip_client
{
  ...
  bool NtripClient::SendHttpRequest()
  {
    ...
    {
      // Read remaining HTTP headers before handing the socket stream to the RTCM parser.
      bytes = boost::asio::read_until(socket_, response, "\r\n\r\n", read_error);
      if (read_error)
      {
        HandleError("Failed to read response headers: " + read_error.message());
        return false;
      }
      ...
      boost::algorithm::to_lower(headers_lower);
      chunked_transfer_ =
          headers_lower.find("transfer-encoding: chunked") != std::string::npos;

      if (debug_)
      {
        RCLCPP_INFO(this->get_logger(), "HTTP headers received:%s%s",
                    headers.empty() ? "" : "\n", headers.c_str());
        RCLCPP_INFO(this->get_logger(), "Chunked transfer decoding: %s",
                    chunked_transfer_ ? "enabled" : "disabled");
      }

      // If any body bytes were read together with the headers, process them now.
      if (response.size() > 0)
      {
        ...
        ProcessTransportBytes(initial_body.data(), initial_body.size());
  ...
  void NtripClient::ProcessTransportBytes(const uint8_t *data, size_t length)
  {
    if (length == 0)
    {
      return;
    ...
    transport_buffer_.append(reinterpret_cast<const char *>(data), length);
    ProcessChunkedTransportBuffer();
  }

  void NtripClient::ProcessChunkedTransportBuffer()
  {
    while (true)
    {
      if (current_chunk_bytes_remaining_ == 0)
      {
        const size_t line_end = transport_buffer_.find("\r\n");
        if (line_end == std::string::npos)
        {
          return;
        }

        std::string chunk_size_line = transport_buffer_.substr(0, line_end);
        const size_t extension_pos = chunk_size_line.find(';');
        if (extension_pos != std::string::npos)
        {
          chunk_size_line.resize(extension_pos);
        }

        char *endptr = nullptr;
        const unsigned long parsed_size =
            std::strtoul(chunk_size_line.c_str(), &endptr, 16);
        if (endptr == chunk_size_line.c_str())
        {
          HandleError("Failed to parse chunk size line: " + chunk_size_line, false);
          transport_buffer_.erase(0, line_end + 2);
          continue;
        }

        transport_buffer_.erase(0, line_end + 2);
        current_chunk_bytes_remaining_ = static_cast<size_t>(parsed_size);

        if (current_chunk_bytes_remaining_ == 0)
        {
          if (debug_)
          {
            RCLCPP_INFO(this->get_logger(), "Received terminating chunk from NTRIP caster");
          }
          transport_buffer_.clear();
          return;
        }
      ...
      if (transport_buffer_.size() < current_chunk_bytes_remaining_ + 2)
      {
        return;
      }

      rtcm_parser_->ProcessData(
          reinterpret_cast<const uint8_t *>(transport_buffer_.data()),
          current_chunk_bytes_remaining_);

      transport_buffer_.erase(0, current_chunk_bytes_remaining_);

      if (transport_buffer_.size() < 2)
      {
        return;
      }

      if (transport_buffer_[0] != '\r' || transport_buffer_[1] != '\n')
      {
        HandleError("Malformed chunk terminator in NTRIP stream", false);
        transport_buffer_.clear();
        current_chunk_bytes_remaining_ = 0;
        return;
      }

      transport_buffer_.erase(0, 2);
      current_chunk_bytes_remaining_ = 0;
    }
  }
