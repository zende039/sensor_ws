...
3. Configure your Python installation. From a command line, run the following
   ...
   `NumPy` is a requirement for PySpin and needs to be at least version `2.0` or above.  
...
2. Install library dependencies for `P`ySpin`: `numpy` and `matplotlib`. `NumPy` is a
   requirement for PySpin and needs to be at least version `1.19` or above for 
   `Ubuntu 20.04` and version `2.0` or above for `Ubuntu 22.04`.  
   `Matplotlib` is not required for the library itself but is used in some of our examples to highlight possible usages of `PySpin`.  
